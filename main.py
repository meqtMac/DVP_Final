...
# audio.plot(fig=fig, ax=axs[0])
noise.plot(fig=fig,ax=axs[0])
# audioAddNoise.plot(fig=fig, ax=axs[0][2])

framed1 = audio.get_framed_audio(800, 800)
# framed1.plot_spectrogram(fig=fig, ax=axs[1])

framed2 = noise.get_framed_audio(800, 800)
framed2.plot_spectrogram(fig=fig, ax=axs[1])
...
for i, process in enumerate(processes):
    ...
    # generatedWave.plot(fig=fig, ax=axs[0])
    generatedFramee = generatedWave.get_framed_audio(800, 800)
    # generatedFramee.plot_spectrogram(fig=fig, ax=axs[1])
    # plt.tight_layout()
